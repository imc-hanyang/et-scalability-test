...
def bind_participant_to_campaign(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select exists( select 1 from "stats"."campaign_participant_stats" where "campaign_id"=%s and "user_id"=%s);',
    ...
    if cur.fetchone()[0]:
        ...
        cur.execute(
            'insert into "stats"."campaign_participant_stats"("user_id", "campaign_id", "join_timestamp")  values (%s,%s,%s) on conflict do nothing;',
            (db_user["id"], db_campaign["id"], utils.timestamp_now_ms()),
        )
        cur.execute(
...
def user_is_bound_to_campaign(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select exists(select * from "stats"."campaign_participant_stats" where "user_id"=%s and "campaign_id"=%s) as "exists";',
...
def register_new_campaign(
    ...
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'insert into "et"."campaign"("creator_id", "name", "notes", "config_json", "start_timestamp", "end_timestamp", "remove_inactive_users_timeout") values (%s,%s,%s,%s,%s,%s,%s);',
        (
            db_user_creator["id"],
            name,
            notes,
...
def update_campaign(
    ...
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'update "et"."campaign" set "name" = %s, "notes" = %s, "config_json" = %s, "start_timestamp" = %s, "end_timestamp" = %s, "remove_inactive_users_timeout" = %s where "id"=%s;',
        (
            name,
            notes,
            configurations,
...
def delete_campaign(db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(f'delete from "et"."campaign" where id=%s;', (db_campaign["id"],))
    cur.close()
    get_db_connection().commit()
...
def get_campaign_participants_count(db_campaign=None):
    ...
    if db_campaign is None:
        ...
        cur.execute(
            'select count(*) as "participant_count" from "et"."user" where "id" in (select "user_id" from "stats"."campaign_participant_stats" where "campaign_id"=%s);',
            (db_campaign["id"],),
        )
...
def get_campaign_participants(db_campaign=None):
    ...
    if db_campaign is None:
        ...
        cur.execute(
            'select * from "et"."user" where "id" in (select "user_id" from "stats"."campaign_participant_stats" where "campaign_id"=%s);',
            (db_campaign["id"],),
        )
    rows = cur.fetchall()
    cur.close()
...
def register_data_source(db_creator_user, name, icon_name):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'insert into "et"."data_source"("creator_id", "name", "icon_name") values (%s,%s,%s) returning "id";',
...
def fast_store_data_record(cur, user_id, campaign_id, data_source_id, timestamp, value):
    cur.execute(
        f'insert into "data"."{campaign_id}-{user_id}"("timestamp", "value", "data_source_id") values (%s,%s,%s) on conflict do nothing returning true;',
        (
            timestamp,
...
def get_next_k_data_records(db_user, db_campaign, from_record_id, db_data_source, k):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        f'select * from "data"."{db_campaign["id"]}-{db_user["id"]}" where "id">=%s and "data_source_id"=%s order by "id" limit({k});',
...
def get_filtered_data_records(
    db_user, db_campaign, db_data_source, from_timestamp, till_timestamp=-1
):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    if till_timestamp > 0:
        cur.execute(
            f'select * from "data"."{db_campaign["id"]}-{db_user["id"]}" where "data_source_id"=%s and "timestamp">=%s and "timestamp"<%s order by "timestamp" asc;',
            (db_data_source["id"], from_timestamp, till_timestamp),
        )
    else:
        cur.execute(
            f'select * from "data"."{db_campaign["id"]}-{db_user["id"]}" where "data_source_id"=%s and "timestamp">=%s order by "timestamp" asc limit 500;',
...
def dump_data(db_campaign, db_user):
    ...
    cur.execute(
        f'copy (select "id", "timestamp", "value", "data_source_id" from "data"."{db_campaign["id"]}-{db_user["id"]}") to %s with binary;',
        (file_path,),
    )
...
def dump_csv_data(db_campaign, db_user=None, db_data_source=None):
    ...
    if db_user is not None:
        ...
        cur.execute(
            f'copy (select "data_source_id", "timestamp", "value" from "data"."{db_campaign["id"]}-{db_user["id"]}") to %s delimiter \',\' csv header;',
            (tmp_file_path,),
        )
        ...
        for index, db_user in enumerate(
            get_campaign_participants(db_campaign=db_campaign)
        ):
            sub_file_path = utils.get_download_file_path(
                f'{db_campaign["id"]}-{db_user["id"]}.csv'
            )
            cur.execute(
                f'copy (select "timestamp", "value" from "data"."{db_campaign["id"]}-{db_user["id"]}" where "data_source_id"={db_data_source["id"]}) to %s delimiter \',\' csv header;',
                (sub_file_path,),
            )
...
def create_direct_message(db_source_user, db_target_user, subject, content):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'insert into "et"."direct_message"("src_user_id", "target_user_id", "timestamp", "subject", "content")  values (%s,%s,%s,%s,%s);',
        (
            db_source_user["id"],
            db_target_user["id"],
...
def get_unread_direct_messages(db_user):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select * from "et"."direct_message" where "target_user_id"=%s and "read"=FALSE;',
        (db_user["id"],),
    )
    rows = cur.fetchall()
    cur.execute(
...
def create_notification(db_target_user, db_campaign, timestamp, subject, content):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'insert into "et"."notification"("target_user_id", "campaign_id", "timestamp", "subject", "content") values (%s,%s,%s,%s,%s)',
...
def get_unread_notifications(db_user):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select * from "et"."notification" where "target_user_id"=%s and "read"=FALSE;',
        (db_user["id"],),
    )
    rows = cur.fetchall()
    cur.execute(
...
def get_participant_join_timestamp(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select "join_timestamp" as "join_timestamp" from "stats"."campaign_participant_stats" where "user_id"=%s and "campaign_id"=%s;',
        (db_user["id"], db_campaign["id"]),
    )
...
def get_participant_last_sync_timestamp(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select max("sync_timestamp") as "last_sync_timestamp" from "stats"."per_data_source_stats" where "campaign_id"=%s and "user_id"=%s;',
        (
            db_campaign["id"],
            db_user["id"],
        ),
...
def get_participant_heartbeat_timestamp(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'select "last_heartbeat_timestamp" as "last_heartbeat_timestamp" from "stats"."campaign_participant_stats" where "user_id" = %s and "campaign_id" = %s;',
        (db_user["id"], db_campaign["id"]),
    )
...
def get_participants_amount_of_data(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        f'select sum("amount_of_samples") as "amount_of_samples" from "stats"."per_data_source_stats" where "campaign_id"=%s and "user_id"=%s;',
        (
            db_campaign["id"],
            db_user["id"],
        ),
...
def get_participants_per_data_source_stats(db_user, db_campaign):
    ...
    for db_data_source in db_data_sources:
        cur.execute(
            f'select "amount_of_samples" as "amount_of_samples" from "stats"."per_data_source_stats" where "campaign_id"=%s and "user_id"=%s and "data_source_id"=%s;',
            (
                db_campaign["id"],
                db_user["id"],
                db_data_source["id"],
        ...
        cur.execute(
            f'select "sync_timestamp" as "sync_timestamp" from "stats"."per_data_source_stats" where "campaign_id"=%s and "user_id"=%s and "data_source_id"=%s;',
            (
                db_campaign["id"],
                db_user["id"],
                db_data_source["id"],
...
def update_user_heartbeat_timestamp(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'update "stats"."campaign_participant_stats" set "last_heartbeat_timestamp" = %s where "user_id" = %s and "campaign_id" = %s;',
        (utils.timestamp_now_ms(), db_user["id"], db_campaign["id"]),
    )
    cur.close()
    get_db_connection().commit()
...
def remove_participant_from_campaign(db_user, db_campaign):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    cur.execute(
        'delete from "stats"."campaign_participant_stats" where "user_id" = %s and "campaign_id" = %s;',
        (db_user["id"], db_campaign["id"]),
    )
    cur.close()
    get_db_connection().commit()
...
def get_filtered_amount_of_data(
    db_campaign, from_timestamp, till_timestamp, db_user=None, db_data_source=None
):
    cur = get_db_connection().cursor(cursor_factory=psycopg2_extras.DictCursor)
    amount = 0
    ...
    if db_user is None:
        ...
        if db_data_source is None:
            ...
            for db_participant_user in get_campaign_participants(
                ...
                cur.execute(
                    f'select count(*) as "amount" from "data"."{db_campaign["id"]}-{db_participant_user["id"]}" where "timestamp">=%s and "timestamp"<%s;',
            ...
            for db_participant_user in get_campaign_participants(
                ...
                cur.execute(
                    f'select count(*) as "amount" from "data"."{db_campaign["id"]}-{db_participant_user["id"]}" where "data_source_id"=%s and "timestamp">=%s and "timestamp"<%s;',
                    (db_data_source["id"], from_timestamp, till_timestamp),
                )
                amount += cur.fetchone()["amount"]
    else:
        ...
        if db_data_source is None:
            # all data sources
            cur.execute(
                f'select count(*) as "amount" from "data"."{db_campaign["id"]}-{db_user["id"]}" where "timestamp">=%s and "timestamp"<%s;',
                (from_timestamp, till_timestamp),
            )
            amount += cur.fetchone()["amount"]
        else:
            ...
            cur.execute(
                f'select count(*) as "amount" from "data"."{db_campaign["id"]}-{db_user["id"]}" where "data_source_id"=%s and "timestamp">=%s and "timestamp"<%s;',
                (db_data_source["id"], from_timestamp, till_timestamp),
            )
            amount += cur.fetchone()["amount"]
    cur.close()
