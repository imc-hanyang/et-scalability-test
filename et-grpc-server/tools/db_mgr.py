...
def get_cassandra_session():
    if settings.cassandra_cluster is None:
        settings.cassandra_cluster = Cluster(
            contact_points=["localhost"], #"et-cassandra"],
...
def set_user_tag(db_user, tag=""):
    session = get_cassandra_session()
    session.execute('update "et"."user" set "tag"=%s where "id"=%s;', (tag, db_user.id))
...
def user_is_bound_to_campaign(db_user, db_campaign):
    ...
    count = session.execute(
        'select count(*) from "stats"."campaignParticipantStats" where "campaignId"=%s and "userId"=%s allow filtering;',
...
def bind_participant_to_campaign(db_user, db_campaign):
    ...
    if not user_is_bound_to_campaign(db_user=db_user, db_campaign=db_campaign):
        session.execute(
            'insert into "stats"."campaignParticipantStats"("userId", "campaignId", "joinTimestamp")  values (%s,%s,%s);',
            (db_user.id, db_campaign.id, utils.get_timestamp_ms()),
        )
        session.execute(
            f'create table if not exists "data"."cmp{db_campaign.id}_usr{db_user.id}"("dataSourceId" int, "timestamp" bigint, "value" blob, primary key ("dataSourceId", "timestamp"));'
        )
...
def get_campaign_participants(db_campaign):
    ...
    for row in session.execute(
        'select "userId" from "stats"."campaignParticipantStats" where "campaignId"=%s allow filtering;',
        (db_campaign.id,),
    ).all():
...
def get_campaign_researchers(db_campaign):
    ...
    db_researchers = []
    for row in session.execute(
        'select "researcherId" from "et"."campaignResearchers" where "campaignId"=%s allow filtering;',
        (db_campaign.id,),
    ).all():
...
def add_researcher_to_campaign(db_campaign, db_researcher_user):
    session = get_cassandra_session()
    session.execute(
        'insert into "et"."campaignResearchers"("campaignId", "researcherId") values(%s,%s);',
...
def remove_researcher_from_campaign(db_campaign, db_researcher_user):
    session = get_cassandra_session()
    session.execute(
        'delete from "et"."campaignResearchers" where "campaignId"=%s and "researcherId"=%s;',
        (db_campaign.id, db_researcher_user.id),
    )
...
def create_or_update_campaign(
    ...
    if db_campaign is None:
        ...
        session.execute(
            'insert into "et"."campaign"("id", "creatorId", "name", "notes", "configJson", "startTimestamp", "endTimestamp") values (%s,%s,%s,%s,%s,%s,%s);',
            (
                next_id,
                db_creator_user.id,
                name,
        ...
        session.execute(
            'update "et"."campaign" set "name" = %s, "notes" = %s, "configJson" = %s, "startTimestamp" = %s, "endTimestamp" = %s where "creatorId"=%s and "id"=%s;',
            (
                name,
                notes,
                configurations,
...
def get_campaign(campaign_id, db_researcher_user=None):
    ...
    if db_researcher_user is None:
        ...
        if db_campaign is None:
            is_researcher = (
                session.execute(
                    'select count(*) from "et"."campaignResearchers" where "campaignId"=%s and "researcherId"=%s;',
                    (campaign_id, db_researcher_user.id),
                ).one()[0]
                > 0
            )
...
def delete_campaign(db_campaign):
    session = get_cassandra_session()
    session.execute(
        f'delete from "et"."campaign" where "creatorId"=%s and "id"=%s;',
        (
            db_campaign.creatorId,
            db_campaign.id,
        ),
    )


def get_campaigns(db_creator_user=None, active_only=False):
    session = get_cassandra_session()
    ...
    if active_only:
        ...
        for db_campaign in db_campaigns:
            if db_campaign.endTimestamp > now_ts:
                res += [db_campaign]
        return res
    return db_campaigns
...
def get_researcher_campaigns(db_researcher_user):
    ...
    db_campaigns = []
    for row in session.execute(
        'select "campaignId" from "et"."campaignResearchers" where "researcherId"=%s allow filtering;',
...
def create_data_source(db_creator_user, name, icon_name):
    ...
    session.execute(
        'insert into "et"."dataSource"("id", "creatorId", "name", "iconName") values (%s,%s,%s,%s);',
...
def store_data_record(db_user, db_campaign, db_data_source, timestamp, value):
    session = get_cassandra_session()
    session.execute(
        f'insert into "data"."cmp{db_campaign.id}_usr{db_user.id}"("dataSourceId", "timestamp", "value") values (%s,%s,%s);',
        (db_data_source.id, timestamp, value),
    )


def store_data_records(
    db_user, db_campaign, timestamp_list, data_source_id_list, value_list
):
    data_sources: dict = {}
...
def get_file(db_campaign, db_user, db_data_source):
...
def store_data_record_to_file(file, timestamp, value):
    ...
    for timestamp, data_source_id, value in zip(
        ...
        if data_source_id not in files:
            ...
            files[data_source_id] = get_file(
                db_campaign=db_campaign, db_user=db_user, db_data_source=db_data_source
            )
...
def get_next_k_data_records(db_user, db_campaign, from_timestamp, db_data_source, k):
    ...
    k_records = session.execute(
        f'select * from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "timestamp">=%s and "dataSourceId"=%s order by "timestamp" asc limit {k} allow filtering;',
...
def get_filtered_data_records(
    db_user, db_campaign, db_data_source, from_timestamp=None, till_timestamp=None
):
    session = get_cassandra_session()
    if None not in [till_timestamp]:
        data_records = session.execute(
            f'select * from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "dataSourceId"=%s and "timestamp">=%s and "timestamp"<%s order by "timestamp" allow filtering;',
            (db_data_source.id, from_timestamp, till_timestamp),
        ).all()
    elif from_timestamp is not None:
        data_records = session.execute(
            f'select * from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "dataSourceId"=%s and "timestamp">=%s order by "timestamp" allow filtering;',
        ...
        data_records = session.execute(
            f'select * from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "dataSourceId"=%s order by "timestamp" allow filtering;',
            (db_data_source.id,),
        ).all()
    return data_records
...
def dump_data(db_campaign, db_user):
    ...
    session.execute(
        f'copy (select "id", "timestamp", "value", "dataSourceId" from "data"."cmp{db_campaign.id}_usr{db_user.id}" allow filtering) to %s with binary;',
        (file_path,),
    )
...
def create_direct_message(db_source_user, db_target_user, subject, content):
    ...
    session.execute(
        'insert into "et"."directMessage"("id", "sourceUserId", "targetUserId", "timestamp", "subject", "content")  values (%s,%s,%s,%s,%s);',
        (
            next_id,
            db_source_user.id,
            db_target_user.id,
...
def get_unread_direct_messages(db_user):
    ...
    db_direct_messages = session.execute(
        'select * from "et"."directMessage" where "targetUserId"=%s and "read"=FALSE allow filtering;',
        (db_user.id,),
    ).all()
    for db_direct_message in db_direct_messages:
        session.execute(
            'update "et"."directMessage" set "read"=TRUE where "targetUserId"=%s and "sourceUserId"=%s and "id"=%s;',
...
def create_notification(db_campaign, timestamp, subject, content):
    ...
    for db_participant in get_campaign_participants(db_campaign=db_campaign):
        session.execute(
            'insert into "et"."notification"("id", "timestamp", "subject", "content", "read", "campaignId", "targetUserId") values (%s,%s,%s,%s,%s,%s,%s)',
            (
                next_id,
                timestamp,
                subject,
...
def get_unread_notifications(db_user):
    ...
    db_notifications = session.execute(
        'select * from "et"."notification" where "targetUserId"=%s and "read"=FALSE allow filtering;',
        (db_user.id,),
    ).all()
    for db_notification in db_notifications:
        session.execute(
            'update "et"."notification" set "read"=TRUE where "campaignId"=%s and "targetUserId"=%s and "id"=%s;',
...
def get_participant_join_timestamp(db_user, db_campaign):
    session = get_cassandra_session()
    res = session.execute(
        'select "joinTimestamp" from "stats"."campaignParticipantStats" where "userId"=%s and "campaignId"=%s allow filtering;',
...
def get_participant_last_sync_timestamp(db_user, db_campaign):
    session = get_cassandra_session()
    res = session.execute(
        'select max("syncTimestamp") from "stats"."perDataSourceStats" where "campaignId"=%s and "userId"=%s allow filtering;',
        (
            db_campaign.id,
            db_user.id,
        ),
...
def get_participant_heartbeat_timestamp(db_user, db_campaign):
    session = get_cassandra_session()
    res = session.execute(
        'select "lastHeartbeatTimestamp" from "stats"."campaignParticipantStats" where "userId" = %s and "campaignId" = %s allow filtering;',
        (db_user.id, db_campaign.id),
    ).one()
...
def get_participants_amount_of_data(db_user, db_campaign):
    ...
    amount_of_samples = cur.execute(
        f'select sum("amountOfSamples") from "stats"."perDataSourceStats" where "campaignId"=%s and "userId"=%s allow filtering;',
        (
            db_campaign.id,
            db_user.id,
        ),
...
def get_participants_per_data_source_stats(db_user, db_campaign):
    ...
    for db_data_source in db_data_sources:
        res = cur.execute(
            f'select "amountOfSamples" from "stats"."perDataSourceStats" where "campaignId"=%s and "userId"=%s and "dataSourceId"=%s allow filtering;',
            (
                db_campaign.id,
                db_user.id,
                db_data_source.id,
        ...
        res = cur.execute(
            f'select "syncTimestamp" from "stats"."perDataSourceStats" where "campaignId"=%s and "userId"=%s and "dataSourceId"=%s allow filtering;',
            (
                db_campaign.id,
                db_user.id,
                db_data_source.id,
        ...
        res_stats += [(db_data_source, amount_of_samples, sync_timestamp)]
    return res_stats
...
def update_user_heartbeat_timestamp(db_user, db_campaign):
    session = get_cassandra_session()
    session.execute(
        'update "stats"."campaignParticipantStats" set "lastHeartbeatTimestamp" = %s where "userId" = %s and "campaignId" = %s;',
...
def remove_participant_from_campaign(db_user, db_campaign):
    session = get_cassandra_session()
    session.execute(
        'delete from "stats"."campaignParticipantStats" where "userId" = %s and "campaignId" = %s;',
        (db_user.id, db_campaign.id),
    )
...
def get_filtered_amount_of_data(
    ...
    if db_user is None:
        ...
        if db_data_source is None:
            # all data sources
            for db_participant_user in get_campaign_participants(
                db_campaign=db_campaign
            ):
                amount += session.execute(
                    f'select count(*) from "data"."{db_campaign.id}-{db_participant_user.id}" where "timestamp">=%s and "timestamp"<%s allow filtering;',
            ...
            for db_participant_user in get_campaign_participants(
                ...
                amount += session.execute(
                    f'select count(*) from "data"."{db_campaign.id}-{db_participant_user.id}" where "dataSourceId"=%s and "timestamp">=%s and "timestamp"<%s allow filtering;',
                    (db_data_source.id, from_timestamp, till_timestamp),
                    timeout=60000,
                ).one()[0]
    else:
        ...
        if db_data_source is None:
            # all data sources
            amount += session.execute(
                f'select count(*) from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "timestamp">=%s and "timestamp"<%s;',
                (from_timestamp, till_timestamp),
                timeout=60000,
            ).one()[0]
        else:
            ...
            amount += session.execute(
                f'select count(*) from "data"."cmp{db_campaign.id}_usr{db_user.id}" where "dataSourceId"=%s and "timestamp">=%s and "timestamp"<%s allow filtering;',
                (db_data_source.id, from_timestamp, till_timestamp),
                timeout=60000,
            ).one()[0]
    return amount
